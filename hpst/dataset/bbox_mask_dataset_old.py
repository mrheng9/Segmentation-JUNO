...
class BboxMaskDataset(Dataset):
    def __init__(self, data_file: str, limit_index=1.0,  transform=None):
        ...
        self.num_segments = 65

        """
        batch = offset2batch(self.hitlim)

        bbox_min_x = scatter(self.coordinates[self.hitview == 0], ((batch[self.hitview == 0]*self.num_segments)+self.object_targets[self.hitview == 0]), 
                           dim_size=self.num_segments*batch[self.hitview == 0].shape[0], reduce="min", dim=0) # (M, 2) -> (64*N, 2)
        print("min done")
        bbox_max_x = scatter(self.coordinates[self.hitview == 0], ((batch[self.hitview == 0]*self.num_segments)+self.object_targets[self.hitview == 0]), 
                           dim_size=self.num_segments*batch[self.hitview == 0].shape[0], reduce="max", dim=0) # (M, 2) -> (64*N, 2)
        print("max done")
        self.bbox_x = torch.cat([bbox_min_x, bbox_max_x], dim=-1) # (64*N, 2), (64*N, 2) -> (64*N, 4)
        print("bbox done")

        bbox_min_y = scatter(self.coordinates[self.hitview == 1], ((batch[self.hitview == 1]*self.num_segments)+self.object_targets[self.hitview == 1]), 
                           dim_size=self.num_segments*batch[self.hitview == 1].shape[0], reduce="min", dim=0) # (M, 2) -> (64*N, 2)
        print("min done")
        bbox_max_y = scatter(self.coordinates[self.hitview == 1], ((batch[self.hitview == 1]*self.num_segments)+self.object_targets[self.hitview == 1]), 
                           dim_size=self.num_segments*batch[self.hitview == 1].shape[0], reduce="max", dim=0) # (M, 2) -> (64*N, 2)
        print("max done")
        self.bbox_y = torch.cat([bbox_min_y, bbox_max_y], dim=-1) # (64*N, 2), (64*N, 2) -> (64*N, 4)


        self.targets = scatter(self.targets, (batch*self.num_segments)+self.object_targets, 
                           dim_size=self.num_segments*batch.shape[0], reduce="max", dim=0) # (M, 1) -> (64*N, 1)
        """
    ...
    def __getitem__(self, item) -> List[Tuple[Tensor, ...]]:
        ...
        targets = self.targets[event_low:event_high]
        ...
        mask_x = object_targets_x >= 0

        coordinates_x_min_val = coordinates_x.min(axis=0, keepdim=True).values
        coordinates_x_max_val = coordinates_x.max(axis=0, keepdim=True).values
        coordinates_x = (torch.Tensor([80-1,100-1]).reshape((1,2)) * ((coordinates_x - coordinates_x_min_val) / (coordinates_x_max_val - coordinates_x_min_val))).to(torch.int64)

        bbox_min_x1 = torch.zeros((object_targets_x.amax()+1,), dtype=coordinates_x.dtype).scatter_reduce_(dim=0, index=object_targets_x[mask_x], src=coordinates_x[:,0][mask_x], reduce="amin") # (M, 2) -> (64, 2)
        bbox_min_x2 = torch.zeros((object_targets_x.amax()+1,), dtype=coordinates_x.dtype).scatter_reduce_(dim=0, index=object_targets_x[mask_x], src=coordinates_x[:,1][mask_x], reduce="amin") # (M, 2) -> (64, 2)
        bbox_max_x1 = torch.zeros((object_targets_x.amax()+1,), dtype=coordinates_x.dtype).scatter_reduce_(dim=0, index=object_targets_x[mask_x], src=coordinates_x[:,0][mask_x], reduce="amax") # (M, 2) -> (64, 2)
        bbox_max_x2 = torch.zeros((object_targets_x.amax()+1,), dtype=coordinates_x.dtype).scatter_reduce_(dim=0, index=object_targets_x[mask_x], src=coordinates_x[:,1][mask_x], reduce="amax") # (M, 2) -> (64, 2)
        bbox_x = torch.stack([bbox_min_x1-1, bbox_min_x2-1, bbox_max_x1+1, bbox_max_x2+1], dim=-1)
        targets_x = torch.zeros((object_targets_x.amax()+1,), dtype=targets_x.dtype).scatter_(dim=0, index=object_targets_x[mask_x], src=targets_x[mask_x])
        ...
        object_targets_x_pos = object_targets_x
        object_targets_x_pos[object_targets_x_pos < 0] = 0

        object_targets_dense_x = torch.nn.functional.one_hot(        
            torch.sparse_coo_tensor(
                coordinates_x.T,
                object_targets_x_pos,
                size=(80,100)
            ).to_dense()
        ).transpose(2,1).transpose(0,1)
        ...
        mask_y = object_targets_y >= 0
        
        targets_y = torch.zeros((object_targets_y.amax()+1,), dtype=targets_y.dtype).scatter_(dim=0, index=object_targets_y[mask_y], src=targets_y[mask_y])

        coordinates_y_min_val = coordinates_y.min(axis=0, keepdim=True).values
        coordinates_y_max_val = coordinates_y.max(axis=0, keepdim=True).values
        coordinates_y = (torch.Tensor([80-1,100-1]).reshape((1,2)) * ((coordinates_y - coordinates_y_min_val) / (coordinates_y_max_val - coordinates_y_min_val))).to(torch.int64)

        bbox_min_y1 = torch.zeros((object_targets_y.amax()+1,), dtype=coordinates_y.dtype).scatter_reduce_(dim=0, index=object_targets_y[mask_y], src=coordinates_y[:,0][mask_y], reduce="amin") # (M, 2) -> (64, 2)
        bbox_min_y2 = torch.zeros((object_targets_y.amax()+1,), dtype=coordinates_y.dtype).scatter_reduce_(dim=0, index=object_targets_y[mask_y], src=coordinates_y[:,1][mask_y], reduce="amin") # (M, 2) -> (64, 2)
        bbox_max_y1 = torch.zeros((object_targets_y.amax()+1,), dtype=coordinates_y.dtype).scatter_reduce_(dim=0, index=object_targets_y[mask_y], src=coordinates_y[:,0][mask_y], reduce="amax") # (M, 2) -> (64, 2)
        bbox_max_y2 = torch.zeros((object_targets_y.amax()+1,), dtype=coordinates_y.dtype).scatter_reduce_(dim=0, index=object_targets_y[mask_y], src=coordinates_y[:,1][mask_y], reduce="amax") # (M, 2) -> (64, 2)
        bbox_y = torch.stack([bbox_min_y1-1, bbox_min_y2-1, bbox_max_y1+1, bbox_max_y2+1], dim=-1)
        ...
        object_targets_y_pos = object_targets_y
        object_targets_y_pos[object_targets_y_pos < 0] = 0
        
        object_targets_dense_y = torch.nn.functional.one_hot(    
            torch.sparse_coo_tensor(
                coordinates_y.T,
                object_targets_y_pos,
                size=(80,100)
            ).to_dense()
        ).transpose(2,1).transpose(0,1)
