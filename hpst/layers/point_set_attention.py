...
class PointSetAttention(nn.Module):
    def __init__(
        ...
        self.qkv2 = nn.Linear(embed_channels, 3*embed_channels, bias=qkv_bias)
    ...
    def forward(self, feat1, coord1, graph1, feat2, coord2, graph2, graph12, graph21):
        ...
        qkv2 = qkv2.reshape(-1, 3, self.num_heads, self.embed_channels//self.num_heads) # (N', V*3*C) => (N', V*3, H, C//H)
        qkv2 = qkv2.permute(1, 0, 2, 3) # (N'2, V*3, H, C//H) => (V*3, N'2, H, C//H)
        query2, key2, value2 = qkv2.unbind(dim=0)
        ...
        if self.training:
            ...
            feat2 = self.calculate_attention_rpe(key2, query2, value2, graph2, coord2, n2)
        else:
            feat1 = self.calculate_attention_rpe_inference(key1, query1, value1, graph1, coord1, n1)
            feat2 = self.calculate_attention_rpe_inference(key2, query2, value2, graph2, coord2, n2)
...
class BlockSequence(nn.Module):
    ...
    def forward(self, points1, points2):
        ...
        graph12 = knn(coord1, coord2, k=self.neighbours, batch_x=batch1, batch_y=batch2)
        graph21 = knn(coord2, coord1, k=self.neighbours, batch_x=batch2, batch_y=batch1)
